...
export default function AnalysisScreen() {
  ...
  const isRefetching =
    overviewRefetching || catRefetching || flowRefetching || accountRefetching;
  const isLoading =
    (overviewLoading || catLoading || flowLoading || accountLoading) &&
    !isRefetching;
  ...
  const net = overview?.net ?? (income - expense);
  ...
  // Expense categories sorted by highest spent
  const expenseCategories = useMemo(() => {
    const items = categoryData?.categories?.filter((c) => c.type === "expense") ?? [];
    return [...items].sort((a, b) => b.total - a.total);
  }, [categoryData]);

  const totalCategoryExpense = useMemo(() => {
    return expenseCategories.reduce((sum, c) => sum + c.total, 0) || expense || 1;
  }, [expenseCategories, expense]);

  // Cash flow days (taking last 7-10 days for bar charts)
  const recentDays = useMemo(() => {
    const days = cashFlowData?.days ?? [];
    return days.slice(-7);
  }, [cashFlowData]);

  // Max daily volume for bar height scaling
